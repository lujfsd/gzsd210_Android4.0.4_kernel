...
static const u16 my_reg[WM8978_CACHEREGNUM]={0,29,405,367,16,0,0,0,0,0,0,
	511,511,0,256,511,255,0,300,44,44,44,269,0,50,0,0,0,0,0,0,0,56,11,50,0,
	8,12,147,233,0,0,0,16,3,440,16,256,256,2,1,1,307,307,313,313,1,1,};
...
static int wm8978_configure_pll(struct snd_soc_codec *codec)
{
	...
	/////////// add by Leslie  ///////
       for(j=0;j<WM8978_CACHEREGNUM;j++)
	{
	      if (j == WM8978_RESET)
			continue;
	      snd_soc_write(codec, j, my_reg[j]);
	}
     ///////////////// end /////////
	return 0;
}
				      ...
				      const struct i2c_device_id *id)
{
	...
	ret = snd_soc_register_codec(&i2c->dev,
			&soc_codec_dev_wm8978, wm8978_dai, ARRAY_SIZE(wm8978_dai));
	if (ret < 0)
		kfree(wm8978);
	return ret;
}
