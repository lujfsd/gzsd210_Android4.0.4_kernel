...
static int novatek_init_panel(struct nt11003_ts_data *ts)
{
...
static int nt11003_init_panel(struct nt11003_ts_data *ts)
{
	...
	int ret=-1;
	uint8_t rd_cfg_buf[7] = {0x8f,};	//I2C buffer start address = 0x78
	...
	if(ret<0)
		dev_info(&client->dev,"read 0x00 error %d",ret);
	dev_info(&client->dev,"Read 0x78 data[0] = %d data[1]=%d data2=%d  data3=%d data 4=%d data5=%d data6 =%d\n",rd_cfg_buf[0],rd_cfg_buf[1],rd_cfg_buf[2],rd_cfg_buf[3],rd_cfg_buf[4],rd_cfg_buf[5],rd_cfg_buf[6]);
#if 0
	rd_cfg_buf[0]=0x8f;
	rd_cfg_buf[1]=0xaf;				  
	ret=i2c_read_bytes(ts->client, rd_cfg_buf, 7);
	if(ret<0)
		dev_info(&client->dev,"read 0xaf error %d",ret);
	dev_info(&client->dev,"Read 0xaf: data[0] = %d data[1]=%d data2=%d  data3=%d data 4=%d data5=%d data6 =%d\n",rd_cfg_buf[0],rd_cfg_buf[1],rd_cfg_buf[2],rd_cfg_buf[3],rd_cfg_buf[4],rd_cfg_buf[5],rd_cfg_buf[6]);
#endif
#if 1	
	if(ret != 2)
	{
		...
		ts->max_touch_num = MAX_FINGER_NUM;
		ts->int_trigger_type = 1;//INT_TRIGGER;
...
#if 1
	dev_info(&client->dev,"Read data[0] = %d data[1]=%d data2=%d  data3=%d data 4=%d data5=%d data6 =%d\n",rd_cfg_buf[0],rd_cfg_buf[1],rd_cfg_buf[2],rd_cfg_buf[3],rd_cfg_buf[4],rd_cfg_buf[5],rd_cfg_buf[6]);
	...
	rd_cfg_buf[0] = 0x7e;//0x7e;
	rd_cfg_buf[1] = 0x00;
	i2c_read_bytes(ts->client, rd_cfg_buf, 2);
	dev_info(&client->dev,"read 0x7e data[1]=%d\n",rd_cfg_buf[1]);
...
{	
	...
	uint8_t  point_data[35]={0x00,};//[(1-READ_COOR_ADDR)+1+2+6*MAX_FINGER_NUM+1]={ 0 };  
		//read address(1byte)+key index(1byte)+point mask(2bytes)+6bytes*MAX_FINGER_NUM+coor checksum(1byte)
	uint8_t  check_sum = 0;
	...
	if(touch_num_make > 0)
	{
		for(index=0; index<touch_num; index++)
		{
			...
			input_report_key(ts->input_dev, BTN_TOUCH , 1);    // pressed
...
{	
	...
	if (error < 0) {
		pr_err("gpio-reset: failed to request GPIO %d,"
...
err_gpio_request_failed:
	...
	ts->input_dev->id.version = 120426;	//screen firmware version
	...
	dev_info(&client->dev,  "Driver Modify Date:2012-12-21\n");
	return 0;
...
static int nt11003_ts_remove(struct i2c_client *client)
{
	...
	dev_info(&client->dev,"The driver is removing...\n");
	i2c_set_clientdata(client, NULL);
