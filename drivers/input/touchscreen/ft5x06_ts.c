...
#define INT_PORT			S5PV210_GPH0(3)
...
{	
	int error = gpio_request(GPIO_RESET,"touch_reset");
	if (error < 0) {
...
static int ft5x0x_read_reg(u8 addr, u8 *pdata)
{
	...
	//printk("this_client->addr is %x",this_client->addr);
	ret = i2c_transfer(this_client->adapter, msgs, 2);
	if (ret < 0)
		pr_err("msg %s i2c read error: %d\n", __func__, ret);
...
static int ft5x0x_read_data(void)
{
	...
	u8 buf[CFG_POINT_READ_BUF] = {0x00};
	int ret = -1;
	int i;
	...
	//printk("read_data buf %d %d %d",buf[0],buf[1],buf[2]);
...
static void ft5x0x_report_value(void)
{
	...
	for (i  = 0; i < event->touch_point; i++)
	{
	    if (event->au16_x[i] < FT5X06_TS_MAX_X && event->au16_y[i] < FT5X06_TS_MAX_Y)
	    // LCD view area
	    {
	    	//printk("%s touch_point[%d] X=%d Y=%d\n",__func__,i,event->au16_x[i],event->au16_y[i]);
	    	//printk("%s touch_event[%d] is %d \n ",__func__,i,event->au8_touch_event[i]);
...
static int ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!ft5x0x_ts->ts_workqueue) {
		...
		err = gpio_request(INT_PORT,"touch_eint");
		if (err < 0) 
		{
	...
	err = request_irq(IRQ, ft5x0x_ts_interrupt, IRQF_TRIGGER_FALLING, "ft5x0x_ts", ft5x0x_ts);
	if (err < 0) {
